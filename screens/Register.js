...
import { StyleSheet, View, Text, ScrollView, Platform, ImageBackground, Alert } from 'react-native';
...
export default function Register({navigation}) {
    ...
    const sub = (year, month, date, user) => {
        let initialDate = new Date(year, month, date);
        let now = Date.now();
        let difference = now - initialDate;
        let millisecondsPerDay = 24 * 60 * 60 * 1000;
        let sinceday = Math.floor(difference / millisecondsPerDay);
    ...
    const registerUser = () => {
        if(fname==''||lname==''||username==''||password==''||email==''||height==''||age==''||weight==''||goal=='') {
            Alert.alert(
                'Registration Failed',
    ...
    return (
        <ScrollView style={styles.view} contentContainerStyle={{flexGrow: 1}}>
            <ImageBackground source={require('../assets/bg4.png')} resizeMode='cover' style={[styles.container]}>
                <View style={{
                    position: 'absolute',
                    top: 40,
                    left: 40
